...
class MongoDBConnector implements Connector
{
    ...
    public async connect (): Promise<void>
    {
        while (true)
        {
            ...
            switch (this.state)
            {
                ...
                case "connecting":
                {
                    throw new FrameworkError("MONGODB_CONNECTOR-CONNECT-IN_CONNECTING_STATE-NO_OPERATION", "The MongoDB connector is currently in the connecting state, but there is no in-flight operation.");
                }
                case "connected":
                {
                    return;
                ...
                case "disconnecting":
                {
                    throw new FrameworkError("MONGODB_CONNECTOR-CONNECT-IN_DISCONNECTING_STATE-NO_OPERATION", "The MongoDB connector is currently in the disconnecting state, but there is no in-flight operation.");
                }
                case "error":
                {
                    break;
    ...
    public async disconnect (): Promise<void>
    {
        while (true)
        {
            ...
            switch (this.state)
            {
                ...
                case "connecting":
                {
                    throw new FrameworkError("MONGODB_CONNECTOR-DISCONNECT-IN_CONNECTING_STATE-NO_OPERATION", "The MongoDB connector is currently in the connecting state, but there is no in-flight operation.");
                }
                case "connected":
                {
                    break;
                ...
                case "disconnecting":
                {
                    throw new FrameworkError("MONGODB_CONNECTOR-DISCONNECT-IN_DISCONNECTING_STATE-NO_OPERATION", "The MongoDB connector is currently in the disconnecting state, but there is no in-flight operation.");
                }
                case "error":
                {
                    break;
    ...
    private async connectCore (): Promise<void>
    {
        ...
        this.logger.log(2, "info", "The MongoDB connector is connecting...");
        ...
        this.logger.log(1, "info", `The MongoDB connector has ${isOk ? "connected" : "failed to connect"}.`);
    }
    ...
    private async disconnectCore (): Promise<void>
    {
        ...
        if (!isPresent(client))
        {
            this.logger.log(2, "info", "The MongoDB connector has no client to disconnect.");
            return;
        }

        this.state = "disconnecting";
        this.logger.log(2, "info", "The MongoDB connector is disconnecting...");
        ...
        this.logger.log(1, "info", `The MongoDB connector has ${isOk ? "disconnected" : "failed to disconnect"}.`);
    }
}
