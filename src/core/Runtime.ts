...
import {isPresent, isPopulated} from "../util/core/value.js";
...
class Runtime
{
    ...
    public constructor (args: RuntimeArgs)
    {
        ...
        this.config = {
            isExitProcess: options.isExitProcess ?? true,
    ...
    public async start (): Promise<void>
    {
        while (true)
        {
            ...
            switch (this.state)
            {
                ...
                case "starting":
                {
                    throw new FrameworkError("RUNTIME-START-IN_STARTING_STATE-NO_OPERATION", "The runtime is currently in the starting state, but there is no in-flight operation.");
                }
                case "running":
                {
                    return;
                ...
                case "stopping":
                {
                    throw new FrameworkError("RUNTIME-START-IN_STOPPING_STATE-NO_OPERATION", "The runtime is currently in the stopping state, but there is no in-flight operation.");
                }
                case "error":
                {
                    break;
    ...
    public async stop (): Promise<void>
    {
        while (true)
        {
            ...
            switch (this.state)
            {
                ...
                case "starting":
                {
                    throw new FrameworkError("RUNTIME-STOP-IN_STARTING_STATE-NO_OPERATION", "The runtime is currently in the starting state, but there is no in-flight operation.");
                }
                case "running":
                {
                    break;
                ...
                case "stopping":
                {
                    throw new FrameworkError("RUNTIME-STOP-IN_STOPPING_STATE-NO_OPERATION", "The runtime is currently in the stopping state, but there is no in-flight operation.");
                }
                case "error":
                {
                    break;
    ...
    private async startCore (): Promise<void>
    {
        ...
        this.state = "starting";
        this.logger.log(2, "info", "The runtime is starting...");
        ...
        this.logger.log(1, "info", `The runtime has ${isOk ? "started" : "started with errors, so it will be stopped."}.`);
    ...
    {
        this.state = "stopping";
        this.logger.log(2, "info", "The runtime is stopping...");
        ...
        this.logger.log(1, "info", `The runtime has ${isOk ? "stopped" : `failed to stop${this.config.isExitProcess ? ", so the process will be exited" : ""}`}.`);

        if (!isOk && this.config.isExitProcess)
        {
            this.exit(1);
        }
    }
    ...
    private logAbort (): void
    {
        ...
        this.logger.log(1, "info", `The runtime has aborted${isPresent(reason) ? ` (${reason})` : ""}.`);
    }
    ...
    private installProcessHandlers (): void
    {
        ...
        this.logger.log(2, "info", "The runtime is installing the process handlers...");
        ...
        this.logger.log(1, "info", "The runtime has installed the process handlers.");
    }
    ...
    private uninstallProcessHandlers (): void
    {
        ...
        this.logger.log(2, "info", `The runtime is uninstalling process handlers...`);
        ...
        this.logger.log(1, "info", "The runtime has uninstalled the process handlers.");
    }
    ...
    private async runInit (): Promise<boolean>
    {
        ...
        this.logger.log(2, "info", "The runtime is running the init hook...");
        ...
        this.logger.log(1, "info", `The runtime has ${isOk ? "run" : "failed to run"} the init hook.`);
    ...
    private async connectConnectors (): Promise<boolean>
    {
        ...
        this.logger.log(2, "info", `The runtime is connecting the connectors...`);
        ...
        this.logger.log(1, "info", `The runtime has ${okCount === totalCount ? "connected" : okCount > 0 ? `connected ${okCount}/${totalCount}` : "failed to connect"} the connectors.`);
    ...
    private async disconnectConnectors (): Promise<boolean>
    {
        ...
        this.logger.log(2, "info", "The runtime is disconnecting connectors...");
        ...
        this.logger.log(1, "info", `The runtime has ${okCount === totalCount ? "disconnected" : okCount > 0 ? `disconnected ${okCount}/${totalCount}` : "failed to disconnect"} the connectors.`);
    ...
    private async startTransports (): Promise<boolean>
    {
        ...
        this.logger.log(2, "info", `The runtime is starting the transports...`);
        ...
        this.logger.log(1, "info", `The runtime has ${okCount === totalCount ? "started" : okCount > 0 ? `started ${okCount}/${totalCount}` : "failed to start"} the transports.`);
    ...
    private async stopTransports (): Promise<boolean>
    {
        ...
        this.logger.log(2, "info", "The runtime is stopping the transports...");
        ...
        this.logger.log(1, "info", `The runtime has ${okCount === totalCount ? "stopped" : okCount > 0 ? `stopped ${okCount}/${totalCount}` : "failed to stop"} the transports.`);
    ...
    {
        this.logger.log(2, "info", `Exiting (code: ${code})...`);
